...
function MarketPage({ me, setMe }) {
  ...
  return <><PageChrome /><div className="main-content"><div className="market-hero card animate-fadeInUp"><div className="card-body"><span className="channel-pill">泓市场</span><h1>{Number(data?.balance || 0).toLocaleString()} <small>泓币</small></h1><p>{me ? '发帖和评论获得泓币，可兑换社区权益。' : '登录后可签到、兑换和查看个人兑换记录；访客可先浏览商品。'}</p>{me ? <button className="btn btn-secondary" onClick={checkin}><i className="fas fa-calendar-check" /> 每日签到</button> : <a className="btn btn-secondary" href="/login"><i className="fas fa-right-to-bracket" /> 登录后兑换</a>}<div className="market-rules">{(data?.rules || []).map(x => <span key={x}>{x}</span>)}</div></div></div>{err && <div className="alert alert-error">{err}</div>}<div className="market-grid">{data?.items?.map(item => <div className="market-card card animate-fadeInUp" key={item.id}><div className="market-icon"><i className={`fas ${item.cover_icon || 'fa-gift'}`} /></div><div className="market-card-body"><h3>{item.title}</h3><p>{item.description}</p><div className="market-meta"><b>{item.price} 泓币</b><span>{item.stock < 0 ? '不限量' : `库存 ${item.stock}`}</span></div><button className="btn btn-primary" disabled={disabledItem(item)} onClick={() => setActiveItem(item)}><i className={`fas ${busy === item.id ? 'fa-spinner fa-spin' : auditTitles.has(item.title) || pendingAuditItemIds.has(item.id) ? 'fa-hourglass-half' : 'fa-bag-shopping'}`} /> {buttonText(item)}</button></div></div>)}</div><div className="card animate-fadeInUp"><div className="card-header fold-head"><h3><i className="fas fa-receipt" /> {me ? '我的兑换记录' : '兑换记录'}</h3><button className="btn btn-sm btn-secondary" onClick={() => setShowOrders(!showOrders)}>{showOrders ? '折叠' : '展开'}</button></div>{showOrders && <div>{data?.orders?.length ? data.orders.map(o => <OrderRow order={o} key={o.id} />) : <div className="empty-state"><i className="fas fa-receipt" /><p>{me ? '暂无兑换记录' : '登录后查看个人兑换记录'}</p></div>}</div>}</div></div><ExchangeModal item={activeItem} balance={data?.balance || 0} busy={Boolean(busy)} onClose={() => setActiveItem(null)} onSubmit={async (item, value) => { await buy(item, value); if (!(item.title === '改名卡' || item.title === '头衔申请券' || item.title === '自定义头衔卡' || isAvatarBorderItem(item))) setActiveItem(null) }} /></>
}
